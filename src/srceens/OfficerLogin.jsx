...
const OfficerLogin = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://hiringapp.vercel.app/api/officer/login",
        { email, password }
      );
      const { message, officer } = response.data;
      Toast.show({
        type: "success",
        text1: message,
      });
      setUserName(officer.fullName);
      setUserEmail(officer.email);
      setId(officer.id);
      setRole("officer");
  ...
  const handleRegister = () => {
    navigation.navigate("OfficerRegister");
  };
